...
| プッシュ 1回 | Enter | 画面中心を目的地にセット |
| プッシュ 2回 | Space | 現在地に戻る |
| プッシュ 長押し | V | 音声入力で目的地検索 |
| プッシュ 3回 | P | 駐車場マーカーの表示切替 |

方向スイッチ A〜D の向きは実装の向きで変わるので、逆だったら
`keymaps/default/keymap.c` の `KC_UP/KC_DOWN/KC_LEFT/KC_RGHT` を入れ替える。
